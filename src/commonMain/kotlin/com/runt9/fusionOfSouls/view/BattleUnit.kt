...
class BattleUnit(val unit: GameUnit, val team: Team) : Container(), EventDispatcher {
    ...
    fun updateHp(amount: Int) {
        ...
        println("[${this.name}]: $hpText for [${abs(amount)}] | From [${previousHp.toIntRound()}] to [${currentHp.toIntRound()}] of [${unit.secondaryAttrs.maxHp.value.toIntRound()}]")
    }
