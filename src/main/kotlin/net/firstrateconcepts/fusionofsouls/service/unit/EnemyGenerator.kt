...
class EnemyGenerator(
    ...
    val enemyPositionPossibilities = ((GAME_WIDTH - COLS_FOR_UNIT_PLACEMENT - 1).toInt()..(GAME_WIDTH - 1).toInt()).flatMap { x ->
    ...
    fun generateEnemies() {
        ...
        val availablePositions = randomizerService.randomize { enemyPositionPossibilities.shuffled().toMutableList() }

        (0 until count).forEach {
            val position = availablePositions.removeFirst()
            // TODO: Ability pool random etc
            val ability = GroundSlamDefinition
            val entity = unitManager.buildUnit("Enemy $it", UnitTexture.ENEMY, ability, UnitType.BASIC, UnitTeam.ENEMY) {
